...
class KyouUnseiModel extends ChangeNotifier {
  ...
  void unsei() {
    ...
    // 説明文１
    // moji[0] = '　今日（$nowMoji）の運勢は・・・';
    // moji[1] = '１．「日干」からみた今日の運勢';
    // moji[2] = '　生年月日の日干：$（$gogyouMoji）';
    ...
    moji[0] = '上の円は日干による運勢、下の円は日支による運勢を表しています。'
        ...
        'の順に星がめぐります。'
        'めぐって来ている星の場所がピンクで表されています。'
        '日支も順にめぐっていきます。こちらも支合・三合・支冲・害・刑が複雑に現れます。'
        'その現れるところがピンクで表されています。'
        'さらに甲と己・丙と辛・戊と癸・庚と乙・壬と丁は干合関係にあり、運勢をおしあげます。'
        '';
    moji[1] = 'おおよその運勢を示すと、'
        ...
        '';
    moji[2] = 'しかし、例えば、凶星の傷官ですが、よく感が働くという点では、感が要求される職場では、吉ともとれますし、'
        '同様に、凶星の偏官も体を痛めるほど仕事が忙しいという点では、例えば客商売のような職場の人にとっては吉ともとれます。'
        '詳細は、各星の内容を見る必要があります。'
        'また、総合で吉でも、支冲や害や刑がある場合は、'
        '売り上げは伸びてもトラブルがあるように吉凶両面含むので、気を付ける必要があります。';
    moji[3] = '１．「日干」からみた $nowMoji の運勢';
    moji[4] = '　　生年月日の日干：壬（みずのえ）';
    moji[5] = '　　　鑑定日の日干：丁（ひのと）';
    moji[6] = '　　鑑定日の通変星：$nowTuhen（$tuhenYomi）';
    // moji[7] = '　　　　　　さらにその内側が（$nikkanInYou）';
    // moji[8] = '　　を表しています。';
    takasaMoji[0] = 300.0;
    takasaMoji[1] = 160.0;
